...
async function verify() {
  try {
    await initDB();
    const result = await client.execute('SELECT COUNT(*) as count FROM parking_slots');
    console.log('Total slots:', result.rows[0].count);
    const slots = await client.execute('SELECT id, slot_number, level FROM parking_slots');
